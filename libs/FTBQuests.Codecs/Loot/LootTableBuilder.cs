...
public sealed class LootTableBuilder
{
    ...
    {
        lootTable = new LootTable(name ?? throw new ArgumentNullException(nameof(name)));
    }
    ...
    public LootTableBuilder AddEntry(Identifier id, int weight, int countMin, int countMax, string? conditions = null)
    {
        ...
        lootTable.Entries.Add(new LootEntry(id, weight, countMin, countMax, normalizedConditions));
        return this;
    }
    ...
    {
        var clone = new LootTable(lootTable.Name);
    ...
    public static LootTable Load(string filePath)
    {
        ...
        var dto = JsonConvert.DeserializeObject<LootTableDto>(json)
                  ?? throw new InvalidDataException("Loot table JSON was empty");
